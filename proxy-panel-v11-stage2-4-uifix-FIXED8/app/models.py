...
class User(UserMixin, db.Model):
    ...
    def get_id(self) -> str:
        """БЕЗОПАСНОСТЬ (аудит 2026-07): идентификатор сессии = id + отпечаток
        текущего пароля.

        Сессия Flask-Login живёт в подписанной cookie, серверного хранилища
        нет. Раньше get_id() (из UserMixin) отдавал только `id`, поэтому
        УТЕКШАЯ cookie оставалась валидной вечно: ни «Выйти» (чистит лишь ту
        cookie, что пришла в запросе), ни смена пароля её не отзывали, а
        12-часовой срок скользящий — при регулярных запросах не истекает
        никогда. Единственным способом выгнать чужого была ручная смена
        SECRET_KEY на сервере.

        Теперь в cookie попадает хвост хеша пароля: сменил пароль → хеш
        изменился → все ранее выпущенные cookie перестают проходить
        load_user. Хвост хеша (не сам хеш) — чтобы не раздувать cookie;
        для инвалидации этого достаточно, а подобрать пароль по 16 символам
        pbkdf2-хеша нельзя.
        """
