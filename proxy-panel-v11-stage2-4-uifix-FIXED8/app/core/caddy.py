...
def generate_caddy_config() -> dict:
    ...
    https_server: dict[str, Any] = {
        ...
        # Стабильность naive на мобильной сети (разбор 2026-07, см.
        # docs/caddy_stability_findings.md).
        #
        # idle_timeout: дефолт Caddy — 5 минут (modules/caddyhttp/app.go).
        # Активный CONNECT-туннель это не рвёт (в HTTP/2 idle-таймер стоит,
        # пока открыт хотя бы один стрим), но когда клиент закрыл все туннели
        # — телефон в кармане — соединение получает GOAWAY уже через 5 минут.
        # При выходе из простоя нужен полный TCP+TLS+h2 handshake, причём
        # ровно тогда, когда мобильная сеть хуже всего. Час позволяет
        # переиспользовать живое соединение.
        # ВАЖНО: значение должно быть ПОЛОЖИТЕЛЬНЫМ. Отрицательное (снятие
        # таймаута) сломает обычные h1-запросы к панели — они получат уже
        # истёкший read-дедлайн.
        "idle_timeout": "1h",
        # keepalive_interval: TCP-пробы держат NAT-маппинг оператора живым
        # между всплесками трафика. Прямое лекарство от диагноза «клиентский
        # health-check не дождался ответа, потому что оператор перебил NAT»
        # (ERR_HTTP2_PING_FAILED в полевом логе без единого признака закрытия
        # со стороны сервера). Дефолт — 15с; берём чуть плотнее.
        "keepalive_interval": "10s",
        # read_timeout / write_timeout СОЗНАТЕЛЬНО НЕ ЗАДАЁМ: в HTTP/2 они
        # становятся PER-STREAM дедлайнами (x/net/http2/server.go) и
        # гарантированно убьют долгий CONNECT-туннель. Не добавлять.
    }
