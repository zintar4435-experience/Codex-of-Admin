...
«сервер↔цель» — соединения, которые процесс учётки открывает наружу по
просьбе клиента: `up` — исходящее от учётки (что клиент отправил), `down` —
ответы цели по `ct mark` (что он получил). Плечо «клиент↔сервер» (порт sshd)
несёт те же самые байты второй раз, и учёт обоих удвоил бы счёт.

Считать по порту sshd **нельзя в принципе**, и это стоило замера на живом
сервере (02.08.2026): `meta skuid` смотрит на владельца СОКЕТА, а сокет
соединения создаёт root'овый sshd вызовом `accept()` до понижения прав —
понижение владельца сокета уже не меняет. Правило `meta skuid <uid> tcp sport
22` не совпадает никогда: при работающем туннеле оно показывало ноль, тогда
как `meta skuid 0 tcp sport 22` считало весь трафик. С таким учётом лимиты
для SSH-клиентов не срабатывали бы вовсе.

Для входящих нужен `ct mark`: сопоставить пакет с владельцем сокета
...
считаются по метке. Фильтра по порту в счётных правилах нет намеренно —
учётка заперта и своей сетевой активности, кроме проброшенных соединений,
иметь не может. При пересборке правил счётчики переносятся, иначе
