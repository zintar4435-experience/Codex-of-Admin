...
### SSH-движок: запертые туннельные учётки

**Зачем.** SSH — единственный протокол, по которому не бьёт объёмная
заморозка соединений (отчёт net4people #490, июнь 2025: SSH и SFTP из-под
неё исключены). Причина не в маскировке, а в цене: по SSH живут хостинги и
разработчики, и широко резать его дороже, чем терпеть.

**Чего он НЕ даёт — честно.** SSH не маскируется вовсе: соединение
начинается с открытой строки `SSH-2.0-…`, и DPI опознаёт его с первого
байта. Это разрешение, а не невидимость. SSH нужен как нога, которая
ломается по ДРУГИМ причинам, чем TLS-протоколы, а не как основной канал.
UDP через SSH не ходит (проброс портов только TCP) — приложения на клиенте
откатываются на TCP.

**Разделение прав — почему панель не делает это сама.** Учётки SSH это
учётки ОС; их заведение и правила учёта трафика требуют root. Панель
работает от непривилегированного `proxypanel`, и её белый список sudoers
сознательно узкий — даже файрволом она только ЧИТАЕТ статус. Выдать ей
`useradd` и `nft` означало бы превратить любую компрометацию процесса
панели в root на хосте. Поэтому:

| Кто | Что делает | Права |
|-----|-----------|-------|
| Панель (`app/core/ssh.py`) | пишет желаемое состояние в `/var/lib/proxy-panel/ssh/state.json`, читает счётчики из `traffic.json` | обычные, без sudo |
| `coc-ssh-sync.py` (systemd-таймер, раз в минуту) | заводит/запирает/удаляет учётки, раскладывает `authorized_keys`, держит счётчики nftables | root |

Синхронизатор НЕ доверяет `state.json`: имя учётки обязано совпасть с
`^cocssh_[0-9]{1,9}$` (трогаем только свои), строка ключа — пройти
регулярку формата authorized_keys, а `subprocess` вызывается списком
аргументов без `shell=True`.

**Цена разделения — задержка.** Изменения (включая отключение по лимиту)
доезжают за один тик таймера, то есть до минуты, а не мгновенно, как у
Xray и Caddy.

**Как заперта учётка.** `/etc/ssh/sshd_config.d/50-coc-tunnel.conf`,
`Match Group cocssh`: без шелла (`ForceCommand /usr/sbin/nologin`), без PTY,
без agent/X11/stream-local-forwarding, пароли выключены. В `authorized_keys`
— опции `restrict,port-forwarding`: `restrict` выключает всё, после чего
проброс портов возвращается явно. Пароля у учётки нет вовсе (`passwd
--lock`), вход только по ключу. Ключи ssh-rsa не принимаются: OpenSSH ≥8.8
по умолчанию отклоняет их подпись, и такой ключ дал бы клиенту молчаливое
«не пускает».

**Учёт трафика.** Ни Xray-stats, ни логов Caddy у SSH нет, поэтому считаем
счётчиками nftables (таблица `inet coc_ssh`). Считается ТОЛЬКО плечо
«клиент↔сервер» (порт sshd): `up` — вход, `down` — выход. Плечо
«сервер↔цель» несёт те же самые байты второй раз, и учёт обоих удвоил бы
счёт. Для входящих нужен `ct mark`: сопоставить пакет с владельцем сокета
(`meta skuid`) ядро умеет только на исходящем пути, поэтому первый
исходящий пакет помечает conntrack-запись меткой учётки, а входящие
считаются по метке. При пересборке правил счётчики переносятся, иначе
добавление клиента обнуляло бы трафик всем остальным.

**Ключи клиента.** Панель генерирует ed25519-пару через `ssh-keygen` при
создании клиента: публичный уходит в `authorized_keys`, приватный — в
ссылку `ssh://…?pk=<base64url PEM>&hk=<ключ сервера>&cv=<баннер>`. Ключи
едут base64url, а не percent-кодированием: в обычном base64 есть `+`,
который на стороне клиента декодируется в пробел и молча ломает ключ.
Приватный ключ хранится в БД панели — как и пароли NaiveProxy, и UUID
VLESS: подписка обязана уметь выдать конфиг повторно. Кто не хочет этого
компромисса — передаёт при создании клиента свой `ssh_public_key`; тогда
приватный не генерируется и не хранится, но ссылку придётся собрать
вручную (API честно вернёт `link: null`, а не ссылку, которая не работает).

**Ограничение: SSH-инбаунд один на сервер.** За ним стоит системный sshd, а
не процесс, который панель поднимает на выбранном порту. Второй инбаунд
...
| `app/core/ssh.py`                          | **новый файл** — SSH-движок со стороны панели: сборка желаемого состояния учёток, атомарная запись `state.json`, чтение счётчиков, генерация ed25519-пары через `ssh-keygen`, чтение публичного ключа хоста |
| `server/coc-ssh-sync.py`                   | **новый файл** — привилегированная половина SSH-движка (root, systemd-таймер): заводит/запирает/удаляет учётки, раскладывает `authorized_keys`, держит счётчики nftables, пишет `traffic.json`. Не доверяет вводу от панели — валидирует всё заново |
